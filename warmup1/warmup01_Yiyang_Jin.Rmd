...
__Special seasons__: None

__Alternative Recipe__: 
...
In [Cartesian coordinates](https://en.wikipedia.org/wiki/Cartesian_coordinate_system), if **p**=($p_1$,$p_2$,.....,$p_n$) and **q** =($q_1$,$q_2$,....,$q_n$) are two points in [Euclidean n-space](https://en.wikipedia.org/wiki/Euclidean_space), then the distance (d) from p to q, or from q to p is given by the [Pythagoran formula](https://en.wikipedia.org/wiki/Pythagorean_theorem).


\[d(\textbf{p},\textbf{q}) = d(\textbf{q},\textbf{p}) = \sqrt{(q_1-p_1)^2+(q_2-p_2)^2+\dots+(q_n-p_n)^2} = \sqrt{\displaystyle\sum_{i=1}^{n}(q_i-p_i)^2}\]
...
\[ \| e \|= \sqrt{p_1^2+p_2^2+\dots+p_n^2}= \sqrt{\textbf{p}\cdot\textbf{p}}\]
